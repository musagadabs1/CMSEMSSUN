<%@ Page Language="C#" MasterPageFile="~/MasterPage.master" AutoEventWireup="true" CodeFile="ATSearch.aspx.cs" Inherits="Pages_ATSearch" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    ...
    <p>
                        ...
                        <asp:RadioButton ID="rdointernal" runat="server" text ="Internal Exam"  AutoPostBack="true"
                            GroupName ="rdointext" Checked="true" 
                            oncheckedchanged="rdointernal_CheckedChanged" />                       
                        <asp:RadioButton ID="rdoexternal" runat="server" text ="External Exam"   AutoPostBack="true"
                            GroupName ="rdointext" oncheckedchanged="rdoexternal_CheckedChanged" />
...
<ItemTemplate>
<asp:LinkButton ID="lnkId" runat="server" CommandName="Modify" CommandArgument='<%# ((GridViewRow) Container).RowIndex %>' ForeColor="Blue"  Font-Underline="true" Width="90%"
           Text='<%# HighlightText(Eval("FirstName").ToString()) %>'></asp:LinkButton>
           <asp:HiddenField ID="hdnlink" runat="server" Value='<%#Eval("linkid").ToString() %>' />
             <asp:HiddenField ID="hdnstatus" runat="server" Value='<%#Eval("teststatus").ToString() %>' />
              <asp:HiddenField ID="hdnexam" runat="server" Value='<%#Eval("etype").ToString() %>' />
</ItemTemplate>
</asp:TemplateField>
...
<ItemTemplate>
<asp:Label ID="lblLocation1" Text='<%#HighlightText(Eval("DegreeCode").ToString()) %>' runat="server" Width="100%" CssClass="buttonface"></asp:Label>
</ItemTemplate>
</asp:TemplateField>
...
<asp:TemplateField HeaderText="TestDone" ItemStyle-Width="20%" >
<ItemTemplate>
<asp:LinkButton ID="lblLocation5" runat="server" CommandName="Modify" CommandArgument='<%# ((GridViewRow) Container).RowIndex %>' ForeColor="Blue"  Font-Underline="true" Width="90%" CssClass="buttonface"
           Text='<%#HighlightText(Eval("teststatus").ToString()) %>'></asp:LinkButton>
...
SelectCommand="ProcGetStudentapptitude"  UpdateCommand="ProcGetStudentapptitude" UpdateCommandType="StoredProcedure"
SelectCommandType="StoredProcedure" 
FilterExpression="teststatus LIKE '%{0}%' or firstName LIKE '%{0}%'  or LastName LIKE '%{0}%' or  Mobileno LIKE '%{0}%' or CallerStatus LIKE '%{0}%' or Degreecode LIKE '%{0}%' or AttendedBy LIKE '%{0}%'  or Emailid LIKE '%{0}%' or calldate1 LIKE '%{0}%' ">
<FilterParameters>
...
<asp:ControlParameter Name="teststatus" ControlID="txtSearch" PropertyName="Text" />
...
<asp:ControlParameter Name="CallDate1" ControlID="txtSearch" PropertyName="Text" />
 ...
 <SelectParameters>
   <asp:SessionParameter Name="empid" SessionField="EMPID1" Type="Int32" />  
      <asp:SessionParameter Name="examtype" SessionField="examtype" Type="String" />  
    </SelectParameters>
